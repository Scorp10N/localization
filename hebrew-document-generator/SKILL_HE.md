...
def add_rtl_paragraph(doc, text, font='David', size=12, bold=False, italic=False,
    ...
    # ללא RIGHT מפורש במקרה ה-RTL — ראו את סעיף "פתרון תקלות" בהמשך: יישור
    # RIGHT מפורש בפסקת `<w:bidi/>` מוצג צמוד-לשמאל ב-LibreOffice (המלכודת
    # של w:jc הלוגי-לא-פיזי, שתועדה במקור רק לתאי טבלה, חלה גם כאן).
...
**רשימות ממוספרות/תבליטים תקינות ל-RTL, וסימוכין `REF` למספרי סעיפים:** אל תשתמשו בסגנון המובנה `'List Bullet'`/`'List Number'` של python-docx במסמך עברי — ההגדרה הממוספרת שלו כוללת הזחה פיזית משמאל ללא מקבילה RTL, ש-LibreOffice/Chromium מסתירים באמצעות שיקוף אוטומטי בתצוגה עד ש-Word האמיתי חושף אותה. אותה בעיה חלה על מספור כותרות אוטומטי ועל ציטוטי שדה `REF`. קוד עבודה מלא, מאומת מול מסמך שנוצר בפועל (לא רק בבדיקה חזותית): `references/rtl-lists-and-fields.md` (באנגלית).
...
### שגיאה: "טקסט עברי — בתאי טבלה, בפסקאות גוף או בכותרות — נדחק לשמאל למרות `<w:bidi/>`"
סיבה: הוגדר יישור `RIGHT` פיזי. `w:jc` ב-OOXML הוא לוגי, לא פיזי: `right` פירושו "סוף השורה", שבפסקה עברית (`<w:bidi/>`) הוא בצד שמאל. תועד במקור רק לתאי טבלה; אומת באמצעות קואורדינטות גליפים מתוך קובץ PDF (לא צילום מסך) שהתקלה חלה גם על פסקאות גוף רגילות ועל כותרות בסגנון `Heading1`/`Heading2` ב-LibreOffice.
פתרון: לעולם אל תגדירו יישור מפורש על פסקת RTL (`<w:bidi/>`), בין אם בגוף המסמך ובין אם בתא טבלה. השארת היישור לא מוגדר מאפשרת ל-`<w:bidi/>` ליישר את הפסקה כברירת מחדל לקצה ה-START שלה (הימין הוויזואלי) — ראו `set_cell_rtl_text` ו-`add_rtl_paragraph`.

...
**Anti-pattern:** never hand-roll `pPr` outside `add_rtl_paragraph`.
...
def _add_runs(p, text, font='David', size=12, bold=False, italic=False, para_has_latin=None):
    """Split text per-script and append runs to paragraph p. para_has_latin
    overrides "does the whole paragraph contain Latin?" for multi-fragment
    paragraphs (e.g. split around a REF field) — see references/rtl-lists-and-fields.md.
    Leave None for the normal single-string case."""
    if para_has_latin is None:
        # rule 2: a paragraph with ANY Latin is "mixed" and never rtl-flags its runs
...
def set_cell_rtl_text(cell, text, font='David', size=11, bold=False):
    ...
    _add_runs(p, text, font=font, size=size, bold=bold)
...
### RTL-correct numbered lists and REF fields

python-docx's built-in `'List Bullet'`/`'List Number'` styles hardcode a LEFT indent — don't use them for Hebrew content; LibreOffice/Chromium hide the bug, Word doesn't. Working code: `references/rtl-lists-and-fields.md`.
...
- `references/rtl-lists-and-fields.md` - RTL-correct numbered lists, heading numbering, REF-field citations, and the OOXML schema-order pitfalls.
...
Solution: `references/rtl-lists-and-fields.md`. Fields need an interactive update (Word/F9), not headless.